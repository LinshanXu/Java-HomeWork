...
public class TestServer {
	public static void main(String[] args) {
		...
		System.out.println("Waiting for gusee:");
		...
		while(count>0){
			try {
				...
				System.out.println("The player guess:"+guess);
				//准备开始判断对错，竞猜次数减一
				count--;
				//用临时变量复制一份结果
				char[] temp = result;
				...
				if(!new String(temp).equals(result)){
					//如果旧结果不等于新结果，说明有猜对的，退还次数
					count++;
				}
//				if(guess.equals(word)){
				if(revert.equals(word)){
					...
					dos.writeUTF(revert);
					//开始新的一轮竞猜
					System.out.println("The player wins,input a new word now.");
					word = scan.next();
					System.out.println("Answer is:"+word);
					System.out.println("Waitring for guess");
					...
					if(count==0){
						score--;
						revert = "\tSorry,no chances left,the answer is:"+word+",score now:"+score;
					}
					dos.writeUTF(revert);
					//关闭资源
					dis.close();
				...
				//次数用完，开始新游戏，重置计数器
				if(count==0){
					System.out.println("The player failed,input a new word.");
					word = scan.next();
					System.out.println("Answer is:"+word);
